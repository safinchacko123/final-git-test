...
class Auth
{
    ...
    function check_access($access, $default_redirect = false, $redirect = false)
    {
        ...
        // Check whether user loged in with customer
        $customer = $this->CI->session->userdata('cart_contents')['customer'];

        $fetchedAccess = '';
        if ($customer['role_id']) {
            if (count($customer['role']) > 0) {
                $fetchedAccess = $customer['role']['role_name'];
            }
    ...
    function is_logged_in($redirect = false, $default_redirect = true)
    {
        ...
        if (!$admin) {
            //check the cookie
            if (isset($_COOKIE['MpCartAdmin'])) {
                //the cookie is there, lets log the customer back in.
                $info = $this->aes256Decrypt(base64_decode($_COOKIE['MpCartAdmin']));
...
//            exit;
    ...
    {
        setcookie('MpCartAdmin', $data, $expire, '/', $_SERVER['HTTP_HOST']);
    }
    ...
    /**
     * Method to return user login details depending upon Customer/Admin session present
     */
    function get_auth(){
        $customer = $this->CI->session->userdata('cart_contents')['customer'];
        $admin = $this->CI->session->userdata('admin');
        $authDtl = array();
        if($customer){
            $authDtl['logedInAs'] = 'Customer';
            $authDtl['id'] = $customer['id'];
            $authDtl['firstname'] = $customer['firstname'];
            $authDtl['lastname'] = $customer['lastname'];
        }else{
            $authDtl['logedInAs'] = 'Admin';
            $authDtl['id'] = $admin['id'];
            $authDtl['firstname'] = $admin['firstname'];
            $authDtl['lastname'] = $admin['lastname'];            
        }
        return $authDtl;
    }
}
