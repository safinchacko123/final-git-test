					...
					<div <?php echo $style; ?> class="divider rounded-field">
						...
						<div class="form-group"> 
							<select  class="form-control" name="listing_delivery_area" id="listing_delivery_area" >
								...
								{
									if(isset($detail['seller_hid_locationName']) && $detail['seller_hid_locationName']==$area['sublocality'])
										$selected = "selected";
									else
										$selected = '';
										
									$ulHTML[] ='<option  '.$selected.' data-lat="'.$area['lat'].'" data-locality="'.$area['locality'].'" data-lng="'.$area['lng'].'" value="'.$area['sublocality'].'" >'.$area['sublocality'].'</option>';
								} 
								echo implode("",$ulHTML);
								?>
							</select>                     
