<?php /*<?php echo form_open_multipart(config_item('admin_folder').'/settings');?>
...
<fieldset>
    <legend><?php echo lang('invoice_settings');?></legend>
    <div class="row">
        <div class="span4">
            <label><?php echo lang('invoice_date');?></label>
            <select class="span3" name="invoice_date">
                <?php for($i=1; $i<=28; $i++) { ?>
                    <option <?php if($invoice_date==$i) { ?> selected="selected" <?php } ?> value="<?php echo $i; ?>"><?php echo $i; ?></option>
                <?php } ?>   
            </select> 
        </div>       
    </div>
