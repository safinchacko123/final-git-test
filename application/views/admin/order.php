        ...
        <?php echo form_open($this->config->item('admin_folder').'/orders/send_notification/'.$order->id);?>
            <fieldset>
                ...
                <select id="canned_messages" onchange="set_canned_message(this.value)" class="span12">
                    ...
                    {
                        if($msg['name']=='Order Shipped Notification' && $this->customer['access']=='Venture')
                        {
                        echo '<option value="'.$msg['id'].'">'.$msg['name'].'</option>';
                        } elseif($this->customer['access']=='') {
                            echo '<option value="'.$msg['id'].'">'.$msg['name'].'</option>';
                    }
                    }
                    ?>
...
<?php echo form_open($this->config->item('admin_folder').'/orders/order/'.$order->id, 'class="form-inline" onsubmit="return validate_orderUpdate_form()" ');?>
<!-- By Lynn 13 May start  -->
<input type="hidden" id="order-login-status" name="order_login_status" value="<?php echo isset($adminSessionDetail['access'])?$adminSessionDetail['access']:''; ?>" />
<!-- By Lynn 13 May end  -->
<fieldset>
    <div class="row" style="margin-top:20px;">
        <div class="span8">
            ...
            <?php if($this->customer['access']=='Venture') { ?>
                <p><?php if($order->notes!='') { echo $order->notes; } else { echo 'NA'; } ?></p>
            <?php } else { ?>
        ...
        <div class="span4">
            ...
            <?php
           // echo form_dropdown('status', $this->config->item('order_statuses'), $order->status, 'class="span4" id="order_status" ');
            ?>
            
            <select <?php if (($this->customer['access'] != '') && (($order->status == 'order_cancelled') || ($order->status == 'order_delivered') || ($order->status == 'order_disputed'))) { ?>disabled="true"<?php } ?> class="span4" name="status">
                <option <?php if ($order->status == 'order_placed') { ?>selected="selected" <?php } ?> value="order_placed">Order Placed</option>
                <option <?php if ($order->status == 'order_in_progress') { ?>selected="selected" <?php } ?> value="order_in_progress">Order in Progress</option>
                <option <?php if ($order->status == 'order_delivered') { ?>selected="selected" <?php } ?> value="order_delivered">Order Delivered</option>
                <option <?php if ($order->status == 'order_disputed') { ?>selected="selected" <?php } ?> value="order_disputed">Order Disputed</option>
                <option <?php if ($order->status == 'order_cancelled') { ?>selected="selected" <?php } ?> value="order_cancelled">Order Cancelled</option>
            </select>
        </div>
    </div>
	<!-- By Lynn 13 May start  -->
     <div class="row order-cancellation-box" style="margin-top:20px;<?php echo (isset($order->status) && $order->status !='order_disputed')?'display:none':''; ?>">
        <div class="span8">
			<h3>Please enter the reason for disbute</h3>
        <textarea name="cancellation_contant" id="cancellation_contant" class="span8"><?php echo (isset($order->status) && $order->status =='order_disputed')?$order->cancellation_contant:''; ?></textarea> 
        </div>
        <div class="span4 ">
