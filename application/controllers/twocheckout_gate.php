...
class twocheckout_gate extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->library(array('mp_cart','Twocheckout'));
	...
	*/
	...
	{
		$settings = $this->mp_cart->CI->Settings_model->get_settings('twocheckout');

		if ($this->mp_cart->CI->input->post('order_number'))
		{
			foreach ($this->mp_cart->CI->input->post() as $k => $v)
			{
				$response[$k] = $v;
			}
		}
		elseif ($this->mp_cart->CI->input->get('order_number'))
		{
			foreach ($this->mp_cart->CI->input->get() as $k => $v)
			{
				$response[$k] = $v;
			}
		}
		...
		{            
			...
			// set a confirmed flag in the payment property
			$this->mp_cart->set_payment_confirmed();
