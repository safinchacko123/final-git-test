...
class Vendors extends Admin_Controller {
    ...
    function index($field = 'lastname', $by = 'ASC', $page = 0) { 
        ...
        $data['vendors'] = $this->Customer_model->get_vendors(50, $page, $field, $by);

        $this->load->library('pagination');

        $config['base_url'] = base_url() . '/' . $this->config->item('admin_folder') . '/vendors/index/' . $field . '/' . $by . '/';
        $config['total_rows'] = $this->Customer_model->count_vendors();
        $config['per_page'] = 50;
        ...
        $this->view($this->config->item('admin_folder') . '/vendors', $data);
    }
	...
	function updateVendorStatus() 
	{
        ...
        if ($vendorId) 
		{ 
		    $result = $this->Customer_model->updateVendorStatus($vendorId);
	...
	function venture($vendorId, $field = 'lastname', $by = 'ASC', $page = 0) { 

        $data['page_title'] = 'Ventures';
        $data['ventures'] = $this->Customer_model->get_ventures($vendorId, 50, $page, $field, $by);

        $this->load->library('pagination');

        $config['base_url'] = base_url() . '/' . $this->config->item('admin_folder') . '/vendors/ventures/' . $field . '/' . $by . '/';
        $config['total_rows'] = $this->Customer_model->count_ventures();
        $config['per_page'] = 50;
        $config['uri_segment'] = 6;
        $config['first_link'] = 'First';
        $config['first_tag_open'] = '<li>';
        ...
        $this->view($this->config->item('admin_folder') . '/ventures', $data);
    }
