...
class Checkout extends Front_Controller {
    ...
    function __construct() {
        ...
        if (!config_item('allow_os_purchase') && config_item('inventory_enabled')) {
            ...
            $inventory_check = $this->mp_cart->check_inventory();
    ...
    function step_1() {
        $data['customer'] = $this->mp_cart->customer();
    ...
    function step_2() {
        ...
        /* now where? continue to step 3 */ else {
            $this->step_3();
        }
    }
    ...
    protected function shipping_form($shipping_methods) {
        ...
        $shipping = $this->mp_cart->shipping_method();
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->mp_cart->set_additional_detail('shipping_notes', $this->input->post('shipping_notes'));
            ...
            /* set shipping info */
            $this->mp_cart->set_shipping($shipping_method[0], $shipping_method[1]->num, $shipping_code);
    ...
    private function _get_shipping_methods() {
        ...
        if (config_item('require_shipping')) {
            ...
            if ($this->mp_cart->requires_shipping()) {
                ...
                if ($this->mp_cart->is_free_shipping()) {
    ...
    function step_3() {
        ...
        if (config_item('require_shipping') && $this->mp_cart->requires_shipping() && $this->_get_shipping_methods()) {
            $code = $this->validate_shipping_option($this->mp_cart->shipping_code());
        ...
        /* now where? continue to step 4 */ else {
            $this->step_4();
        }
    }
    ...
    protected function payment_form($payment_methods) {
        ...
        $data['customer'] = $this->mp_cart->customer();
        $data['shipping_method'] = $this->mp_cart->shipping_method();

        /* are the being bounced back? */
        $data['payment_method'] = $this->mp_cart->payment_method();
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->mp_cart->set_payment($module, $this->$module->description());
    ...
    function place_order() {
        ...
        $payment = $this->mp_cart->payment_method();
        ...
        $contents = $this->mp_cart->contents();
        if (empty($contents)) {
            redirect('cart/view_cart');
        } else {
            //  - check to see if we have a payment method set, if we need one
            if (empty($payment) && $this->mp_cart->total() > 0 && (bool) $payment_methods == true) {
                redirect('checkout/step_3');
            }
        }
        ...
        if (!empty($payment) && (bool) $payment_methods == true) {
            ...
            if ($this->mp_cart->total() > 0 && !isset($payment['confirmed'])) {
                //run the payment
        ...
        // save the order
        $order_id = $this->mp_cart->save_order();

        $data['order_id'] = $order_id;
        $data['shipping'] = $this->mp_cart->shipping_method();
        $data['payment'] = $this->mp_cart->payment_method();
        $data['customer'] = $this->mp_cart->customer();
        $data['shipping_notes'] = $this->mp_cart->get_additional_detail('shipping_notes');
        $data['referral'] = $this->mp_cart->get_additional_detail('referral');

        $order_downloads = $this->mp_cart->get_order_downloads();
        ...
        $data['mp_cart']['group_discount'] = $this->mp_cart->group_discount();
        $data['mp_cart']['subtotal'] = $this->mp_cart->subtotal();
        $data['mp_cart']['coupon_discount'] = $this->mp_cart->coupon_discount();
        $data['mp_cart']['order_tax'] = $this->mp_cart->order_tax();
        $data['mp_cart']['discounted_subtotal'] = $this->mp_cart->discounted_subtotal();
        $data['mp_cart']['shipping_cost'] = $this->mp_cart->shipping_cost();
        $data['mp_cart']['gift_card_discount'] = $this->mp_cart->gift_card_discount();
        $data['mp_cart']['total'] = $this->mp_cart->total();
        $data['mp_cart']['contents'] = $this->mp_cart->contents();

        /* remove the cart from the session */
        $this->mp_cart->destroy();
